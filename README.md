...
### 3. Kafka 소비자 멱등성 — at-least-once 대응

Kafka at-least-once 보장으로 동일 이벤트가 재전달될 수 있음. Kafka를 소비하는 모든 서비스(inventory, order, payment, notification)에 `processed_events` 테이블 도입.

```sql
CREATE TABLE processed_events (
    id          BIGINT AUTO_INCREMENT PRIMARY KEY,
    kafka_topic VARCHAR(30) NOT NULL,  -- 토픽명 (ORDER_CREATED, PAYMENT_SUCCESS 등)
    order_id    BIGINT      NOT NULL,
    UNIQUE (kafka_topic, order_id)
);
```
